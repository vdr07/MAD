...
- `jpetstore` and `react` Analysis (16h), analyzing both `jpetstore` and `react` which both include multiple decompositions that eventually timeout after 4h, via the command: `./jpetstore_benchmark.sh`
...
Once both steps have been executed, the reviewer may visualize the results by once again starting the Jupyter Notebook as depicted in previous steps.



Both previous steps combined contain all the results for executing 
## Manual Installation
